...
> ⚠️ **【已归档 · 2026-09-22】本文件描述的是 v10.1 方案，已被 v12.2 取代，不得作为施工依据。**
>
> 本文件通篇建立在 **v10.1 的库存 4 态状态机 + StockLog 流水 + 软删除回收站 + 8 跳迁移链**之上。
> 这套方案已在 v12.2 被明确推翻：**D1**（余量降为单一数字，打卡减 / 撤销加，不做流水与状态机）、
> **D2**（硬删除 + 二次确认，不做回收站）。今天 `src/` 里既没有 `stockService.ts`，也没有 `deletedAt` 字段。
>
> **本文档的 §5 红线、§6 Phase 0–8、§7 必需测试用例、§9 进度记录（110 用例）、§10 偏差清单均描述已不存在的实现**，
> 仅供追溯「当时为什么那样设计」。
>
> | 你要找什么                     | 去看哪个文件                                              |
> | ------------------------------ | --------------------------------------------------------- |
> | 当前需求基线                   | `../v12.0-需求重梳理.md`（v12.2，唯一权威）               |
> | 当前施工图                     | `../实施指导书-AI执行版.md`（红线 / 算法规格 / 页面规格） |
> | 当前视觉规范                   | `../UI设计提示词-v1.0.md`                                 |
> | 实施过程中的一切偏离与自行决定 | `../DECISIONS.md`                                         |
> | v10.1 需求原文（被本文件引用） | `../v10.1.md`                                             |
>
> 保留理由：本文档 §2 技术栈取舍、§3 目录结构、§4 需求→代码映射表的**方法论**仍有参考价值。

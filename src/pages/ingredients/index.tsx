  ...
  return (
    <div className="mx-auto w-full max-w-[720px] p-6">
      ...
      {!loading && rows.length === 0 ? (
        <EmptyState
          title="还没有成分"
          description="先建成分，再到补剂里关联「每份含多少」，今日页就会出现成分汇总"
                ...
                return (
                  <Fragment key={ingredient.id}>
                    <TableRow>
                      ...
                      <TableCell className="tabular-nums">{ingredient.upperLimit ?? '—'}</TableCell>
                      <TableCell>
                        {sources.length === 0 ? (
                      ...
                      <TableCell className="text-right">
                        <Button size="sm" variant="outline" onClick={() => setEditing(ingredient)}>
                          编辑
                        </Button>
                      </TableCell>
                    </TableRow>
